...
pitching_path = os.path.join(base_dir, "pitching_combined_all.csv")
...
pitching_df["name"] = pitching_df["name"].apply(clean)
pitching_df["team"] = pitching_df["team"]
...
pitching_df.rename(columns={"Full Name": "Full Team Name"}, inplace=True)

# If still missing, use the mapping CSV
for idx, row in pitching_df[pitching_df["Full Team Name"].isna()].iterrows():
    old_team = row["team"]
    ...
    if not mapping_match.empty:
        ...
        if not matched_team.empty:
            pitching_df.at[idx, "Full Team Name"] = matched_team.iloc[0]["Full Name"]
...
for _, row in tqdm(pitching_df.iterrows(), total=len(pitching_df), desc="Matching players"):
...
pitching_df["Round"] = rounds
pitching_df["Pick"] = picks
pitching_df["Drafted By"] = drafted_by
pitching_df["Drafted From"] = drafted_from
pitching_df["Drafted?"] = drafted_flags

# === Output ===
output_path = os.path.join(base_dir, "PitchingDraftTablev2.csv")
